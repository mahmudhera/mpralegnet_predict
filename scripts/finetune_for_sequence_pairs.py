#!/usr/bin/env python
"""
Predict SNP differential MPRA activity (Δ = alt - ref) from paired sequences using a pretrained MPRA-LegNet.

Implements three methods:

(A) frozen-embedding + ridge:
    - freeze pretrained LegNet
    - extract per-allele embeddings (penultimate vector before head)
    - build delta features: d = h_alt - h_ref
    - fit ridge regression (closed form) on train, select alpha on val, eval on test

(B) siamese-delta-head:
    - shared LegNet encoder E(.)
    - predict Δ via small MLP head g(h_alt - h_ref)
    - option: freeze encoder (default) for stability with 5-10k pairs
    - train on train, select best epoch on val, eval on test

(C) soft-classification regression for Δ:
    - same siamese encoder, but head outputs logits over Δ bins
    - train with soft labels (Gaussian around true Δ)
    - prediction = expectation over bin centers
    - select best epoch on val, eval on test

Input data format:
- TSV/CSV with at least:
    ref sequence column
    ref activity column (log2)
    alt sequence column
    alt activity column (log2)

Example:
    python scripts/delta_train.py \
...
Notes:
- IMPORTANT: splitting is by PAIR (row), not by individual sequences.
- Optional: pairwise reverse-complement augmentation (applies same orientation to ref+alt).
- Optional: rc_average eval averages predictions from forward + reverse-complement.
"""
...
def read_pairs_table(
    ...
    *,
    ...
    with path.open("r", newline="") as f:
        ...
        missing = [c for c in [ref_seq_col, ref_activity_col, alt_seq_col, alt_activity_col] if c not in reader.fieldnames]
        if missing:
        ...
        for row in reader:
            ...
            ry = float(row[ref_activity_col])
            ay = float(row[alt_activity_col])
            ref_seqs.append(rs)
            alt_seqs.append(asq)
            ref_y.append(ry)
            alt_y.append(ay)
    ...
    if len(ref_seqs) != len(alt_seqs):
        raise SystemExit("Ref/alt sequence row counts differ (unexpected).")
...
def collect_delta_embeddings(
    ...
    *,
    ...
    # optional reverse loader for rc_average
    rev_loader = None
    if rc_average:
...
def ridge_fit_closed_form(
    ...
    *,
    ...
    D = X.shape[1]
    XtX = Xc.t().mm(Xc)
    XtX = XtX + alpha * torch.eye(D, dtype=XtX.dtype)
    Xty = Xc.t().mv(yc)
    ...
    def _predict(loader0: DataLoader) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for x_ref, x_alt, y in loader0:
            ...
            with torch.cuda.amp.autocast(enabled=amp and device.type == "cuda"):
                ...
                p = softcls_predict_expectation(logits, centers)
            preds.append(p.detach().cpu())
            ys.append(y.detach().cpu())
        return torch.cat(preds, dim=0).view(-1), torch.cat(ys, dim=0).view(-1)
...
def run_softcls(
    ...
    *,
    ...
    # infer embed dim
    encoder.eval()
    x_ref0, _, y0 = next(iter(train_loader))
    x_ref0 = x_ref0.to(device)
    with torch.no_grad():
        h0 = encoder(x_ref0)
    embed_dim = int(h0.shape[-1])
...
def main() -> None:
    ...
    soft.add_argument("--num_bins", type=int, default=161, help="Number of Δ bins (default: 161)")
    ...
    ref_seqs, ref_y, alt_seqs, alt_y = read_pairs_table(
        data_path,
        sep=args.sep,
    ...
    # Filter: remove rows where |delta| < 0.01
    filtered_indices = []
    ...
    n_removed = len(ref_seqs) - n_pairs
    print(f"Filtered pairs: removed {n_removed} rows (|delta| < 0.01)")
    print(f"  final number of pairs: {n_pairs}")
