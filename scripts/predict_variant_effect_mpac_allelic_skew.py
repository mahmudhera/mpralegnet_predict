...
def main() -> None:
    ...
    inp.add_argument("--input", type=str, required=True, help="tsv with sequences where columns are variant,chrom,pos,ref_log2FC,alt_log2FC,ref_seq,alt_seq,single_nucleotide_difference,log2FC_skew")
    
    out = parser.add_argument_group("output")
    out.add_argument("--output", type=str, required=True, help="Output csv path")
    ...
    ids = df['variant'].astype(str).tolist()
    ...
    if len_of_each_sequence > seq_len:
        ...
        alt_seqs = [seq[trim_amount:trim_amount + seq_len] for seq in alt_seqs]

    # Create datasets
    ref_dataset_fwd = SequenceOnlyDataset(ids, ref_seqs, seq_len=seq_len, reverse=False, add_reverse_channel=add_reverse_channel)
    alt_dataset_fwd = SequenceOnlyDataset(ids, alt_seqs, seq_len=seq_len, reverse=False, add_reverse_channel=add_reverse_channel)
    ...
    if args.rc_average:
        ref_dataset_rev = SequenceOnlyDataset(ids, ref_seqs, seq_len=seq_len, reverse=True, add_reverse_channel=add_reverse_channel)
        alt_dataset_rev = SequenceOnlyDataset(ids, alt_seqs, seq_len=seq_len, reverse=True, add_reverse_channel=add_reverse_channel)
